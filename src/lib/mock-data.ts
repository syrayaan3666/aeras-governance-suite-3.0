...
export const getAcademicExceptions = async (): Promise<AcademicException[]> => {
  const query = `SELECT Id, Name, ExceptionType_c__c, Status_c__c, Institutional_Risk_Score_c__c, Risk_Category_c__c, Policy_ID_Applied_c__c, Grievance_Probability_c__c, SLA_Deadline_c__c, CreatedDate, LastModifiedDate, Is_System_Initiated_c__c, Denial_Reason__c FROM Academic_Exception_c__c ORDER BY CreatedDate DESC LIMIT 50`;
  ...
  return records.map(record => ({
    ...
    denialReason: record.Denial_Reason__c || undefined
  }));
};
...
// NOTE: Governance logs are read-only and reflect actions taken by Salesforce Flow.
// No governance logging is performed from UI code.
...
// Governance feed reflects Salesforce Flow automation outcomes
export const governanceFeed = [
  {
    id: 'feed-1',
    timestamp: new Date(Date.now() - 5 * 60 * 1000),
    type: 'CASE_CREATED',
    message: 'Attendance threshold breach detected for section ENGR-301-2. Case auto-generated by Salesforce Flow.',
    source: 'SALESFORCE_FLOW',
  ...
  },
  {
    ...
    type: 'SLA_WARNING',
    message: 'Case AE-2024-00847 approaching SLA deadline. 8 hours remaining. Monitored by Salesforce Flow.',
    source: 'SALESFORCE_FLOW',
  ...
  },
  {
    ...
    type: 'ESCALATION',
    message: 'Case AE-2024-00831 escalated to Provost Office by Salesforce Flow due to risk threshold.',
    source: 'SALESFORCE_FLOW',
  ...
  },
  {
    ...
    message: 'Risk score updated for 3 cases by Salesforce Flow based on historical pattern analysis.',
    source: 'SALESFORCE_FLOW',
    caseNumber: null
  },
  {
    ...
    type: 'SLA_BREACH',
    message: 'Case AE-2024-00789 breached 48hr SLA. Escalation handled by Salesforce Flow automation.',
    source: 'SALESFORCE_FLOW',
